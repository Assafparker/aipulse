...
7. **כהה הוא ברירת המחדל, ללא תנאי.** מבקר בלי העדפה שמורה מקבל כהה, יהיו
   הגדרות מערכת ההפעלה אשר יהיו. `prefers-color-scheme` אינו משתתף בהחלטה
   ואינו מופיע כלל ב-`globals.css`. הדבר היחיד שמשנה זאת הוא ההחלפה הידנית,
   שכותבת `data-theme` על `<html>` ושומרת ב-localStorage. הגדר את **כל**
   הפלטה על `:root` בערכי הכהה, ודרוס רק ב-`:root[data-theme="light"]`.
   **שים לב:** גם `toggle()` ב-`theme-toggle.tsx` חייב ליפול חזרה ל-`"dark"`
   כשאין `data-theme` — אחרת הקליק הראשון במערכת בהירה לא יעשה דבר.
